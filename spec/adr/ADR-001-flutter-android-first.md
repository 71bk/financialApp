...
- applicationId 目前為 `com.financialapp.financial_app`（Flutter 依 org＋專案名產生，
  已一致套用於 gradle namespace／applicationId、MainActivity package 與 kotlin 資料夾路徑）。
  產品名仍為暫名，正式名稱定案後上架前再改一次（applicationId 上架後不可變更）。
